    ...
    <footer style={{ backgroundColor: '#030813', color: '#64748B', padding: '5rem 0 3rem', fontSize: '12.5px', borderTop: '1px solid rgba(255,255,255,0.06)' }}>
      <div className="public-container" style={{ margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '3rem', marginBottom: '4rem' }}>
          ...
          <div>
            <h4 style={{ color: '#ffffff', fontFamily: 'var(--font-title)', fontSize: '14px', fontWeight: '600', marginBottom: '1.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Dịch vụ</h4>
            <Link to="/members" style={{ display: 'block', padding: '6px 0', color: '#94A3B8', textDecoration: 'none', transition: 'color 0.2s' }}>Doanh nghiệp hội viên</Link>
            <Link to="/posts" style={{ display: 'block', padding: '6px 0', color: '#94A3B8', textDecoration: 'none', transition: 'color 0.2s' }}>Thị trường giao thương</Link>
            <Link to="/ai-chat" style={{ display: 'block', padding: '6px 0', color: '#94A3B8', textDecoration: 'none', transition: 'color 0.2s' }}>Khám phá trợ lý AI</Link>
          </div>
          <div>
            <h4 style={{ color: '#ffffff', fontFamily: 'var(--font-title)', fontSize: '14px', fontWeight: '600', marginBottom: '1.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Hỗ trợ</h4>
            <Link to="/posts" style={{ display: 'block', padding: '6px 0', color: '#94A3B8', textDecoration: 'none', transition: 'color 0.2s' }}>Bản tin Cơ hội</Link>
            <Link to="/register" style={{ display: 'block', padding: '6px 0', color: '#94A3B8', textDecoration: 'none', transition: 'color 0.2s' }}>Hướng dẫn đăng ký</Link>
            <a href="/#tiers" style={{ display: 'block', padding: '6px 0', color: '#94A3B8', textDecoration: 'none', transition: 'color 0.2s' }}>Quyền lợi hội viên</a>
          </div>
          <div>
        ...
        <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '1.5rem', fontSize: '12px' }}>
          ...
          <div style={{ display: 'flex', gap: '20px' }}>
            <Link to="/members" style={{ color: '#64748B', textDecoration: 'none' }}>Hội viên</Link>
            <Link to="/posts" style={{ color: '#64748B', textDecoration: 'none' }}>Bảng tin</Link>
            <Link to="/register" style={{ color: '#64748B', textDecoration: 'none' }}>Gia nhập</Link>
            <Link to="/ai-chat" style={{ color: '#64748B', textDecoration: 'none' }}>Trợ lý AI</Link>
          </div>
        </div>
      </div>
    </footer>
